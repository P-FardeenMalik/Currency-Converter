...
# Create an SQLite database and a table for user accounts
db_connection = sqlite3.connect("user_accounts.db")
...
class Convert:
    ...
    def __init__(self, root, username):
        ...
        self.lbl_Title = Label(Title_frame, text="Currency Converter | Foreign Exchange Rates", padx=17, pady=4, bd=1,
                               font=('arial', 30, 'bold'), bg="#02A9FC", width=50)
        self.lbl_Title.pack()
        ...
        self.Converted = Scale(Frame_3, variable=var1, from_=0, to=1000, length=500, tickinterval=50, orient=VERTICAL, bg='#EA2A97', label="Convert",
                               font=('arial', 10, 'bold'))
        self.Converted.grid(row=0, column=0, rowspan=2)

        self.INR = Scale(Frame_1, variable=var2, from_=0, to=1000, length=500, tickinterval=50, orient=VERTICAL, bg='#EA2A97', label="INR",
                         font=('arial', 10, 'bold'))
        ...
        self.Option['values'] = ("Select", "Argentine Peso", "Australian Dollar", "Bahraini Dinar", "Batwana Pula",
                                 "Brazilian Real", "British Pound", "Bruneian Dollar", "Bulgarian Lev", "Canadian Dollar",
                                 "Chilean Peso", "Chinese Yuan Renmimbi", "Colombian Peso", "Crotian Kuna", "Czech Koruna",
                                 "Danish Krone", "Emirati Dirhan", "Euro", "Hong Kong Dollar", "Hungarian Foint", "IceLandic Krona",
                                 "Indonesian Rupiah", "Iranian Rial", "Israeli Shekel", "Japanese Yen", "Kazakhstani Tenge",
                                 "Kuwaiti Dinar", "Libyan Dinar", "Malaysian Ringgit", "Mauritian Rupee", "Mexican Peso",
                                 "Nepalese Rupee", "New Zealand Dollar", "Norwegian Krone", "Omani Rial", "Pakistani Rupee",
                                 "Philippine Peso", "Polish Zloty", "Qatari Riyal", "Romanian New Leu", "Russian Ruble",
                                 "Saudi Arabian Riyal", "Singapore Dollar", "South African Rand", "South Korean Won",
                                 "Sri Lankan Rupee", "Swedish Krona", "Swiss Franc", "Taiwan New Dollar", "Thai Baht",
                                 "Trinidadian Dollar", "Turkish Lira", "US Dollar", "Venezuelan Bolivar", "")
        ...
        self.btnconvert = Button(Frame_2ButtomR, text="Convert", padx=2, pady=8, bd=2, fg="black", font=('arial', 20, 'bold'),
                                 width=14, height=2, command=Converted_value)
...
# Create a function to check and validate user credentials
def check_credentials():
    ...
    if result:
        ...
        # Create the Converter window with the valid username
        root = Tk()
        application = Convert(root, username)
        root.mainloop()
    else:
        tkinter.messagebox.showerror("Invalid Credentials", "Invalid username or password.")
...
def create_account():
    ...
    if not username or not password:
        tkinter.messagebox.showerror("Error", "Both username and password are required.")
    else:
        try:
            db_cursor.execute("INSERT INTO users (username, password) VALUES (?, ?)", (username, password))
            db_connection.commit()
            tkinter.messagebox.showinfo("Success", "Account created successfully. You can now log in.")
...
login_window.configure(background="#F2C496")
...
create_account_button = Button(login_window, text="Create Account", command=create_account)
